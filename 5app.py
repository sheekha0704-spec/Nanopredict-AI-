...
if nav == "Step 1: Sourcing":
    ...
    # 3 Main Options Selection
    input_mode = st.radio(
        "Select Sourcing Method:",
    ...
    with c1:
        if input_mode == "Choose Drug from Database":
            ...
            st.info(f"Selected: {drug_choice}")
            # Placeholder for profile logic if drug selected from DB
            st.session_state.current_profile = {"logp": 2.5, "mw": 200, "hbd": 1} # Default fallback
            ...
            if up_file:
                ...
                st.success("File Uploaded Successfully! Reloading Database...")
                st.rerun()
            ...
            if smiles and RDKIT_AVAILABLE:
                ...
                if mol:
                    ...
                    st.write(f"**LogP:** {logp:.2f} | **MW:** {mw:.2f}")
                else:
    ...
    with c2:
        ...
        if df is not None and 'current_profile' in st.session_state:
            ...
            st.info(f"Recommended matches (LogP Ref: {p['logp']:.2f}):")
